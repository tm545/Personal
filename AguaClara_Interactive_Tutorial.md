...
![MyPet](./Images/MyPet.png)
<img
src="https://github.com/tm545/Personal/blob/master/Images/MyPet.png?raw=true" height=200 width=250>
...
<!--- Fill you answer here. --->
BE is a useless major with no job opportunities don't apply [here](http://beadvised.bee.cornell.edu/)
...
|:-------------------------- |:------ | --------------------------- | ---------------- |
| Cats                       | apples | Textbook1                   | Upson            |
| dogs                       | pears  | Textbook2                   | Clark            |
| the animal who made github | steak  | atom uninstall readmefile | Olin             |
...
def function(value):
  if value == 1:
    ...
    print('Your value is neither 1 nor 10')
```
...
def diffusion_calculation(T, E, R):
  T = T * u.k
  E = E * u.kg / u.m * u.s
  R = R * u.m
  D = (T * kB)/(6*np.pi*E*R).to_base_units()
  return D


diffusion_calculation(297,5,4)
...
```python
from aide_design.play import*
myTemp = np.array(np.linspace(273,473,200))

myViscosity = np.array(pc.viscosity_kinematic(myTemp))
Area = np.pi*.2*.2
myRey = np.array(pc.re_pipe(2/Area,.4,myViscosity))
myRey[0]
plt.plot(myTemp,myRey)
plt.xlabel('Temperature(Kelvin)')
plt.ylabel('Reynolds Numbers')
plt.title('Reynolds Numbers with Varying Temperature')
plt.minorticks_on()
plt.grid(which = 'major')
plt.grid(which = 'minor')
plt.savefig('./Images/ReynoldsNumbers.png')
plt.show()
...
Hope you're enjoying being a new member! -Desiree
Don't delete my messages next time Tigran. -Philip
